...
static constexpr char system_prompt[] =
    ...
    "Use bounded Python for calculations that benefit from code. Print each "
    "value needed for the answer. When the user asks for a plot, call "
    "run_python with 2 to 24 matching literal x and y points and a short "
    "title. Use null for an undefined y point. Always include a null point "
    "between the two sides of a discontinuity so the line has a visible gap, "
    "even when the undefined x value was not otherwise sampled. Do not give "
    "Python code for a plot. "
...
inline const char *routing_prompt() {
    static const std::array<char, 2'048> prompt = [] {
        ...
        const int size = std::snprintf(
            ...
            "result ID. Use run_python for numeric calculations and plots. "
            "Python must print each value needed for a calculation answer. "
            "When the user asks for a plot, call run_python with a plot object "
            "that has 2 to 24 "
            "matching literal x and y points and a short title. Use null for an "
            "undefined y point. Always include a null point between the two "
            "sides of a discontinuity so the line has a visible gap, even when "
            "the undefined x value was not otherwise sampled. Do not give "
            "Python code for a plot. One run_python call completes routing. Use "
            "get_device_status before a "
