...
void test_idle_timer_starts_after_playback_completion() {
    InteractionStateMachine machine;
    machine.ready(0);
    ...
    machine.tick(50'000);
    TEST_ASSERT_EQUAL_INT(
        static_cast<int>(InteractionState::speaking),
        static_cast<int>(machine.state()));
    machine.interaction_finished(60'000);
    machine.tick(89'999);
    TEST_ASSERT_EQUAL_INT(
        static_cast<int>(InteractionState::idle),
        static_cast<int>(machine.state()));
...
void test_error_starts_idle_timer_when_failure_occurs() {
...
void test_error_idle_timer_handles_millisecond_wrap() {
    InteractionStateMachine machine{InteractionConfig{350, 30'000, 2'200}};
    const std::uint32_t error_at =
        std::numeric_limits<std::uint32_t>::max() - 99U;
    machine.ready(error_at - 1U);
    machine.fail(error_at);
    machine.tick(2'099);
    TEST_ASSERT_EQUAL_INT(
        static_cast<int>(InteractionState::error),
        static_cast<int>(machine.state()));
...
void test_mode_button_accepts_only_a_short_complete_press() {
    chatesp::ShortPressGesture button;
    TEST_ASSERT_FALSE(button.release(10));
    button.press(100);
    TEST_ASSERT_TRUE(button.release(800));
    button.press(900);
    TEST_ASSERT_FALSE(button.release(929));
    button.press(1'000);
    TEST_ASSERT_TRUE(button.release(1'030));
    button.press(2'000);
...
int main(int, char **) {
    ...
    RUN_TEST(test_error_starts_idle_timer_when_failure_occurs);
    RUN_TEST(test_error_idle_timer_handles_millisecond_wrap);
